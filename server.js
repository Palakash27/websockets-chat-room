    ...
    socket.on("message", (data) => {
        ...
        switch (type) {
            case CLIENT.MESSAGE.NEW_USER:
                ...
                const dataWithTime = {
                    type: SERVER.BROADCAST.NEW_USER_WTIH_TIME,
                    payload,
                };
...
function broadcast(data, socketToOmit) {
    ...
    wsServer.clients.forEach((connectedSockets) => {
        if (
            connectedSockets.readyState === WebSocket.OPEN &&
            connectedSockets !== socketToOmit
        )
            connectedSockets.send(data);
    });
}
